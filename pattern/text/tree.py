...
# Chonks are non-chunks,
# see also the chunked() function:
class Chonk(Chunk):
    def __repr__(self):
        return Chunk.__repr__(self).replace("Chunk(", "Chonk(", 1)
...
# s.chunked(s)     => [Chunk('black cats/NP'), Chonk('and/O'), Chunk('white dogs/NP')]

def chunked(sentence):
    """ Returns a list of Chunk and Chonk objects from the given sentence.
        Chonk is a subclass of Chunk used for words that have Word.chunk == None
...
XML_CHoNK    = "chonk"    # <chonk>, where word.chunk.type=None.
...
def parse_xml(sentence, tab="\t", id=""):
    ...
    for word in sentence.words:
        ...
        if chunk and chunk.start == word.index:
            ...
            indent = push(indent)
        # Words outside of a chunk are wrapped in a <chonk> tag:
        # <chonk>
        if not chunk:
            xml.append(indent + '<%s>' % XML_chonk)
            indent = push(indent)
        ...
        if not chunk:
            # Close the <chonk> element if outside of a chunk.
            indent = pop(indent); xml.append(indent + "</%s>" % XML_CHONK)
...
def parse_string(xml):
    ...
    for sentence in dom(XML_SENTENCE):
        ...
        # Traverse all <chunk> and <chonk> elements in the sentence.
...
def _parse_tokens(chunk, format=[WORD, POS, CHUNK, PNP, REL, ANCHOR, LEMMA]):
    ...
    """
    tokens = []
    # Only process <chunk> and <chonk> elements, 
    # text nodes in between return an empty list.
    if not (chunk.tag == XML_CHUNK or chunk.tag == XML_CHONK):
        return []
...
def nltk_tree(sentence):
    ...
    for ch in sentence.chunked():
        if not ch.pnp and isinstance(ch, Chonk):
